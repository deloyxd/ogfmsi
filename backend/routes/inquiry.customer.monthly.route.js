...
router.put('/customers/monthly/:id', async (req, res) => {
  const { id } = req.params;
  const { customer_tid } = req.body;

  const query = `
    UPDATE customer_monthly_tbl 
    SET customer_tid = ?, customer_pending = 0
    WHERE customer_id = ? AND customer_pending = 1
  `;

  mysqlConnection.query(query, [customer_tid, id], (error, result) => {
    if (error) {
