...
document.addEventListener('ogfmsiAdminMainLoaded', function () {
  if (main.sharedState.sectionName !== 'store-inventory') return;
...
function sectionTwoMainBtnFunction() {
  const searchInput = document.getElementById('store-inventorySectionTwoSearch');
  const searchValue = searchInput.value;inventory

  main.findAtSectionOne('store-inventory', searchValue, 'search', 1, (result) => {
    ...
    main.findAtSectionOne('store-inventory', product.dataset.id, 'equal', 2, (pendingResult) => {
      if (pendingResult) {
...
if (quantityValue === 0) {
  ...
  main.createAtSectionOne('store-inventory', columnsData, 1, productName, (generated) => {
    const action = {
      module: 'Store',
      submodule: 'Inventory',
    ...
    main.createRedDot('store-inventory', 1);
...
function processCheckinUser(product) {
  ...
  main.createAtSectionOne('store-inventory', columnsData, 5, '', () => {
    const action = {
      module: 'Store',
      submodule: 'Inventory',
    ...
    main.createRedDot('store-inventory', 2);
