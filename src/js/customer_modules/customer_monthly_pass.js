/*
  Gym Monthly Pass Registration Flow
  - Info → Registration → Confirmation (Custom modal, project style)
  - Ready for future API integration (FormData prepared with files)
*/

(function membershipPassModule() {
  ...
  function openPaymentModal(preparedRegistrationData) {
    ...
    document.getElementById('mpPaySubmit').addEventListener('click', () => {
      ...
      const normalizeAmount = (val) => {
        // remove any currency symbols and spaces, keep digits and dot
        const cleaned = String(val).replace(/[^0-9.]/g, '');
        // handle multiple dots gracefully by taking first two parts
